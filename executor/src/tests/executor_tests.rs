...
#[tokio::test]
async fn test_executor_certificates_restored() {
    let (tx_executor, rx_executor) = channel(10);
    let (tx_output, mut rx_output) = channel(10);
    ...
    // Create additional certificates that don't make it to the executor because it crashed.
    let certificates = test_u64_certificates(
        /* certificates */ 3, /* batches_per_certificate */ 3,
        /* transactions_per_batch */ 3,
    );

    let mut restored_messages = Vec::new();
    for (certificate, _batches) in certificates {
        let message = ConsensusOutput {
            certificate,
            consensus_index: SequenceNumber::default(),
        };
        restored_messages.push(message);
    }
    let _ = rx_output.recv().await;
    ...
    // Feed certificates with good transactions to the executor.
    let certificates = test_u64_certificates(
        /* certificates */ 3, /* batches_per_certificate */ 3,
        /* transactions_per_batch */ 3,
    );
